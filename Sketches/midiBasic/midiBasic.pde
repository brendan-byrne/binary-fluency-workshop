/* -------------------------------------
 USB MIDI Processing Basic Example
...
void noteOn(Note note) {
  ...
  if (pitch == 48 && vel > 0) { // toggle with other if statement below
  // if (x >= width) {
    x = -globalSize;
    y+=globalSize;
    if (y >= height) {
...
void trianglez(int x, int y, int bigness, int type) {
  pushMatrix();
  translate(x, y);
  bigness += random(80);
  ...
  if (type < 4) {
    ...
    switch (type) {
      ...
      case 5:
        ...
        break;
      case 6:
        rect(0+bigness/4, 0+bigness/4, .5*bigness, .5*bigness);
        break;
      case 7:
